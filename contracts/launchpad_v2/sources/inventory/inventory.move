...
//     use nft_protocol::utils::{Self};
//     use ob_launchpad_v2::factory::{Self, Factory};
...
//     struct InventoryDfKey has copy, store, drop {}

//     /// A type-erased wrapper around `Warehouse` and `Factory` which
//     /// allows us to keep the inner object whiile regulating shared access to it
...
//         df::add(&mut id, InventoryDfKey {}, warehouse);

//         Inventory { id }
//     }

//     /// Create a new `Inventory` from a `Factory`
//     public fun from_factory<T: key + store>(
//         _witness: DelegatedWitness<T>,
//         factory: Factory<T>,
//         ctx: &mut TxContext,
//     ): Inventory<T> {
//         let id = object::new(ctx);
//         df::add(&mut id, InventoryDfKey {}, factory);
