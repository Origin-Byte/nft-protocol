...
module ob_tests::orderbook_migration {
    ...
    fun migrate_orderbook_bids() {
        ...
        // Assert that orderbook state
        assert!(crit_bit_v1::max_key(orderbook_v1::borrow_bids(&book_v1)) == 300, 0);
        assert!(crit_bit_v1::min_key(orderbook_v1::borrow_bids(&book_v1)) == 1, 0);
        assert!(crit_bit_v1::length(orderbook_v1::borrow_bids(&book_v1)) == 300, 0);
        ...
        while (i > 0) {
            ...
            orderbook_v1::cancel_bid_permissionless(
                &mut book_v1,
                price,
                buyer(),
                ctx(&mut scenario),
        ...
        // For convenience we are just transfering the kiosk to the owner
        // but this should be shared
        transfer::public_transfer(buyer_kiosk, buyer());
