...
module nft_protocol::sui_safe {
    ...
    public fun transfer_nft_to_recipient<I: key + store, IW: drop, E: drop, T: key + store>(
        ...
        let nft = get_nft_for_transfer_<I, E, T>(entity_witness, entity_id, nft_id, safe);
    ...
    public fun delist_nft<I: key + store, IW: drop, E: drop>(
        ...
        assert_authorised_entity(&as_request<E>(object::uid_to_inner(entity_id)), &auth);
    }
    ...
    public fun check_auth<E: drop>(request: &Request<E>, auth: &TransferAuth): bool {
        let check_1 = request.entity_id == auth.entity_id;
    ...
    public fun as_request<E: drop>(entity_id: ID): Request<E> {
        Request {
            entity_id,
        }
    }
